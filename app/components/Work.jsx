...
const Work = ({isDarkMode}) => {
  return (
    <motion.div 
      ...
      id='work' className='w-full px-[12%] py-10'> {/* Removed scroll-mt, fixed quotes */}
        <motion.h4 
                ...
                <motion.div 
                  ...
                  style={{backgroundImage: `url(${project.bgImage})`}}>
                    <div className='bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex items-center justify-between duration-500 group-hover:bottom-7'>
                        <div>
                            <h2 className='font-semibold flex items-center'>{project.title}</h2>
                            <p className='text-sm text-gray-700 flex items-center'>{project.description}</p>
                        </div>
